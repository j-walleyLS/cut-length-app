...
def parse_boq_text(text):
    ...
    # For technical drawings, try to find quantity patterns and associate with dimensions
    full_text = ' '.join(lines)
    
    # First, try to find technical drawing patterns (e.g., "50no." followed by dimensions)
    drawing_patterns = [
        # Matches "50no." or "50 no." followed by dimensions
        r'(\d+)\s*no\.?\s*.*?(\d{3,4})[^\d]+(\d{3,4})',
        # Matches "50no." with dimensions on separate lines
        r'(\d+)\s*no\.?\s*\n\s*(\d{3,4})\s+(\d{3,4})',
        # Matches quantities with "nos", "pcs", etc.
        r'(\d+)\s*(?:nos?|pcs?|units?)\.?\s*.*?(\d{3,4})[^\d]+(\d{3,4})',
    ]
    
    # Try technical drawing patterns first
    for pattern in drawing_patterns:
        matches = re.finditer(pattern, full_text, re.IGNORECASE | re.MULTILINE | re.DOTALL)
        for match in matches:
            try:
                qty, dim1, dim2 = match.groups()
                qty = int(qty)
                dim1 = int(dim1)
                dim2 = int(dim2)
                
                # Validate dimensions are reasonable (between 100mm and 10000mm for technical drawings)
                if 100 <= dim1 <= 10000 and 100 <= dim2 <= 10000 and qty > 0:
                    # Determine width and height (larger dimension is typically width)
                    width = max(dim1, dim2)
    ...
    # If we found units from technical drawing patterns, return them
    if units:
        return units
    
    # Otherwise, fall back to standard BOQ text parsing
    for line in lines:
        line = line.strip()
        if not line:
            continue
        ...
        skip_patterns = [
            ...
            r'(?i)\*groove', r'(?i)locations', r'(?i)section', r'(?i)views',
            r'(?i)approximate'
        ]
        ...
        patterns = [
            ...
            # Technical drawing patterns for individual lines
            r'(\d+)\s*no\.?\s*
...
def extract_text_with_cloud_ocr(pdf_bytes, progress_callback=None):
    ...
    try:
        ...
        for page_num in range(total_pages):
            ...
            # Convert page to image - reduced DPI for faster processing
            mat = fitz.Matrix(100/72, 100/72)  # Further reduced to 100 DPI
            pix = page.get_pixmap(matrix=mat)
            img_data = pix.pil_tobytes(format="PNG")
            ...
            # Save as JPEG with lower quality for faster upload
            image.save(output, format='JPEG', quality=50, optimize=True)
            compressed_data = output.getvalue()
            
            # If still too large, compress more
            if len(compressed_data) > 300000:  # 300KB limit
                output = BytesIO()
            ...
            while retry_count < max_retries:
                try:
                    ...
                    payload = {
                        ...
                        'scale': False,
                        'OCREngine': 1,  # Engine 1 is faster
                        'isTable': False,
                        'filetype': 'JPG'
                    }
                    
                    response = requests.post(api_url, data=payload, timeout=30)  # Reduced timeout
                    ...
                    if response.status_code == 200:
                        ...
                        if result.get('IsErroredOnProcessing') == False:
                            ...
                            if 'limit' in str(error_msg).lower():
                                st.warning("OCR API limit reached. Try again later or use manual input.")
                                return None
                    ...
                    retry_count += 1
                    if retry_count < max_retries:
                        st.warning(f"OCR timeout for page {page_num + 1}. Retrying...")
                        time.sleep(1)
                        continue
                    else:
                        st.warning(f"OCR failed for page {page_num + 1} after {max_retries} attempts.")
                        continue
                        
                except Exception as e:
                    st.warning(f"OCR error on page {page_num + 1}: {str(e)}")
                    break
        ...
        if not combined_text.strip():
            ...
            st.info("""
            • The document might be an image-heavy technical drawing
            • OCR API rate limits or timeout issues
            • Poor image quality or complex layouts
            
            **Please use the manual input below to enter your data.**
            
            For technical drawings, you can type:
            ```
            x50 900×200
            x37 900×450
            ```
...
if uploaded_file is not None:
    ...
    with col2:
        if st.button("Clear Extract", type="secondary", use_container_width=True):
...
if slab_sizes and st.session_state.units:
    ...
    st.info("👆 Please select slab sizes and add finished units to begin optimization."),  # Just quantity on a line (e.g., "50no.")
        ]
        
        # Check if this line is just a quantity indicator
        qty_match = re.match(r'(\d+)\s*no\.?\s*
...
if slab_sizes and st.session_state.units:
    ...
    st.info("👆 Please select slab sizes and add finished units to begin optimization."), line, re.IGNORECASE)
        if qty_match:
            # Look for dimensions in the next few lines
            qty = int(qty_match.group(1))
            line_idx = lines.index(line)
            
            # Look ahead for dimension patterns
            for i in range(1, min(5, len(lines) - line_idx)):
                next_line = lines[line_idx + i].strip()
                # Look for two numbers that could be dimensions
                dim_match = re.findall(r'\b(\d{3,4})\b', next_line)
                if len(dim_match) >= 2:
                    try:
                        dim1 = int(dim_match[0])
                        dim2 = int(dim_match[1])
                        if 100 <= dim1 <= 10000 and 100 <= dim2 <= 10000:
                            width = max(dim1, dim2)
                            height = min(dim1, dim2)
                            units.append({
                                'width': width,
        ...
        for pattern in patterns[:-1]:  # Exclude the last pattern which we handled above
